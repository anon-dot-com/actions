...
const navigateToLinkedInMessagingPage = async (
  networkHelper: NetworkHelper,
  page: Page,
) => {
  ...
  await networkHelper.takeScreenshot(page, "linkedin", "1-messaging-homepage");
...
const searchAndSelectRecipient = async (
  networkHelper: NetworkHelper,
  page: Page,
  recipient: string,
) => {
  ...
  console.log("Selecting recipient from search results...");
  await networkHelper.retryWithBackoff(async () => {
...
const sendMessage = async (
  ...
  console.log("Confirming message sent...");
  await Promise.race([
    networkHelper.waitForNetworkIdle(page),
    networkHelper.waitForSelector(page, 'div[aria-label="Message sent"]'),
    networkHelper.waitForPageLoad(page),
  ])
    .then(() => console.log("Message confirmation received."))
    .catch(() => console.log("Message confirmation timeout, but proceeding."));
  await networkHelper.takeScreenshot(
    page,
    "linkedin",
